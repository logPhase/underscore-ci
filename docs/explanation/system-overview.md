...
2. **Run analysis** (`id: analyze`) — resolves `mode: auto` to `pr` (on `pull_request`; on any other event it still resolves to `pr` when the dispatched ref is the head branch of an open PR — the payload a pull_request run would carry is synthesized from the API and the resolved PR rides to the publish step as `resolved-*` outputs) or `full`, derives `INTENT_DRIFT_REPO_ID` (default: the repo name) and `INTENT_DRIFT_WORKSPACE` (default: the `workspace` input, else the org), then `docker run`s the image with the mounts a container action would have received.
