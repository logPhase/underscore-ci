...
Three components under `src/components/journeys/` and `src/components/bpmn/` have no live importers at all: `CallFlowChart.tsx`, `bpmn/BpmnFlow.tsx` and `bpmn/KnowledgePanel.tsx`. `BpmnFlow` is a React-Flow implementation of the same diagram that nothing mounts; it still shares `layout.ts` (via `flow-graph.ts`) with the live canvas, so layout changes are covered by `flow-graph.test.ts` even though the component itself is dead. `KnowledgePanel` (and the 📚 journey-knowledge markers it served) was retired when the diagram's annotation layer moved to living specs — the live path is the § marker on PR-changed elements (`BpmnNode`'s `SpecBadge`, fed by `BpmnElement.spec_refs`) opening `bpmn/SpecPopup.tsx`, a centered modal that shows the analyzer-verified verdict + requirement quote and the full spec text from the baked specs payload (`spec-refs.ts` resolves it; a sibling repo's spec falls back to the quote alone). Edit `BpmnCanvas` when you mean to change what users see.
